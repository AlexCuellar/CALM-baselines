...
class DMPs(object):
    ...
    def __init__(self, n_dmps, n_bfs, dt=.01,
                 y0=0, goal=1, runtime=1.0, w=None,
    ...
    def rollout(self, timesteps=None, goal=None, y0=None, perturb=None, **kwargs):
        ...
        produced_path = np.zeros((timesteps, self.n_dmps))
        for t in range(timesteps):
            ...
            # if perturb != None:
            #     if not already_perturbed and t*self.dt > perturb["time"]:
            #         print("PERTURBED!!!!!!")
            #         self.y = perturb["dist"]
            #         already_perturbed = True
            print("PERTURB: ", perturb)
            if perturb != "" and t*self.dt > perturb["time_start"] and t*self.dt < perturb["time_end"]:
                print("PERTURBED!!!!!!")
                if not already_perturbed:
                    perturb_step = self.dt*(perturb["dist"] - y_track[t-1])/(perturb["time_end"] - perturb["time_start"])
                dy_track[t] = perturb_step
                y_track[t] = y_track[t-1] + perturb_step
                
                self.y = y_track[t]
                self.dy = dy_track[t]
                # DON"T WORRY ABOUT SETTING self.ddy, IT DOES NOT MATTER FOR CALCULATING THE NEXT FORCE IN step() 
                already_perturbed = True        
            
            produced_path[t,:] = self.y
